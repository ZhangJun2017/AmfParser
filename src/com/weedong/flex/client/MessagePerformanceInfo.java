...
/**
 * @exclude The MessagePerformanceInfo class is used to capture various metrics about
...
public class MessagePerformanceInfo implements Serializable, Cloneable {
    ...
    /**
     * @exclude Size of message in Bytes (message types depends on what header this MPI is in)
     */
    public long messageSize;

    /**
     * @exclude Millisecond timestamp of when this message was sent
     * (origin depends on on what header this MPI is in)
     */
    public long sendTime;

    /**
     * @exclude Millisecond timestamp of when this message was received
     * (destination depends on on what header this MPI is in)
     */
    public long receiveTime;

    /**
     * @exclude Amount of time in milliseconds that this message was being processed on the server
     * in order to calculate and populate MPI metrics
     */
    public long overheadTime;

    /**
     * @exclude "OUT" when this message originated on the server
     */
    public String infoType;

    /**
     * @exclude True if this is info for a message that was pushed from server to client
     */
    public boolean pushedFlag;

    /**
     * @exclude Flag is true when record-message-sizes is enabled for the communication channel
     */
    public boolean recordMessageSizes;

    /**
     * @exclude Flag is true when record-message-times is enabled for the communication channel
     */
    public boolean recordMessageTimes;

    /**
     * @exclude Millisecond timestamp of when the server became ready to push this message out
     * to clients
     */
    public long serverPrePushTime;

    /**
     * @exclude Millisecond timestamp of when the server called into the adapter associated with the
     * destination of this message
     */
    public long serverPreAdapterTime;

    /**
     * @exclude Millisecond timestamp of when server processing returned from the adapater associated
     * with the destination of this message
     */
    public long serverPostAdapterTime;

    /**
     * @exclude Millisecond timestamp of when the adapter associated with the destination of this message
     * made a call to an external component (for example a JMS server)
     */
    public long serverPreAdapterExternalTime;

    /**
     * @exclude Millisecond timestamp of when processing came back to the adapter associated with the destination
     ...
     * @exclude Copies the immutable fields of this MPI isntance over to create a new one
     */
    public Object clone() {
        try {
            return super.clone();
     ...
     * @exclude Increase the overhead counter for this MPI
     */
